...
This code is to accompany the book entitled
"Beginning Arduino ov7670 Camera Development".

Please refer to the book for detailed explainations 
of how to use this code, how to use the Omnivision ov7670 camera as well
as how to set up the camera with the Arduino and an SD Card.
...
// Pino de Leitura do LDR e a Sensibilidade
#define PinLDR A0
#define VarLuz 900
...
void ResetCameraRegisters()
{
  ...
  byte data = ReadRegisterValue(COM7); // Retorna o valor no endereço do registrador COM7 da memoria da cÂmera através de uma comunicação i2c. 
...
String CreatePhotoInfo()
{
  ...
  // Info = Command + " " + FPSParam + " " + AWBParam + " " + AECParam + " " + YUVMatrixParam + " " + DenoiseParam + " " + EdgeParam + " " + ABLCParam;

  Info = String(now.day()) + '/' + String(now.month()) + '/' + String(now.year()) + " - " + DiasDaSemana[now.dayOfTheWeek()] + " - " + String(now.hour()) + ':' + String(now.minute()) + ':' + String(now.second());
...
void setup()
{
     ...
     if (! rtc.isrunning()) {
        Serial.println("RTC não esta ajustado, ajustando RTC...!");
        // following line sets the RTC to the date & time this sketch was compiled
        // rtc.adjust(DateTime(F(__DATE__), F(__TIME__)));
        // This line sets the RTC with an explicit date & time, for example to set
        // January 21, 2014 at 3am you would call:
        // rtc.adjust(DateTime(2019, 5, 12, 17, 04, 0));
      }
      else{
        Serial.println("RTC já ajustado");
        // rtc.adjust(DateTime(2019, 5, 13, 0, 04, 53));
      }
...
{   
     ...
     while (1)
     {
         ...
         else if (LeituraLDR < VarLuz)  
         {
             lcd.clear();
             lcd.setCursor(0,0);
             lcd.print("Aguardando...");
             Serial.println("Aguardando...");
             while(Botao == 0 && analogRead(PinLDR) < VarLuz && Serial.available() == 0){Botao = digitalRead(BTN);} // Segura aqui enquanto o usuário não apertar o botão
             delay(200);
             ...
             if (Botao == 1)
             {
                 lcd.clear();
                 lcd.setCursor(0,0);
                 lcd.print("Se posicione e ");
                 lcd.setCursor(0,1);
                 lcd.print("aperte novamente");
                 Serial.println("Se posicione e aperte novamente");
                 while(Botao == 1){Botao = digitalRead(BTN);} // Segura aqui se o botão for mantido pressionado
                 delay(200);
                 while(Botao == 0 && analogRead(PinLDR) < VarLuz){Botao = digitalRead(BTN);} // Segura aqui enquanto o usuário não apertar o botão
                 delay(200);
                 if (Botao == 1)
                 {
                     lcd.clear();
         ...
         else if (LeituraLDR > VarLuz)
         {
             ...
             Serial.println("Ambiente Escuro, ligue a luz");
             while(LeituraLDR > VarLuz){LeituraLDR = analogRead(PinLDR);}
             delay(200);        
         }         
      }
